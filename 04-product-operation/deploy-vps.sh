#!/usr/bin/env bash
# ============================================================================
#  ECP Digital Food — Deploy VPS (Padronizado)
#  Ubuntu 22.04+ | Bash 5+
#
#  USO:
#    scp deploy-vps.sh root@<IP>:/root/deploy-food.sh
#    ssh root@<IP>
#    chmod +x /root/deploy-food.sh && bash /root/deploy-food.sh
#
...
APP_NAME="ecp-digital-food"
REPO_URL="https://github.com/ecportilho/ecp-digital-food.git"
REPO_DIR="/opt/ecp-digital-food"
APP_CWD="/opt/ecp-digital-food/03-product-delivery"
APP_PORT=3000
NODE_VERSION="20"
...
# INICIO
# ============================================================================
banner "ECP Digital Food — Deploy VPS"
...
echo ""
info "Integracao com ecp-digital-bank (mesma VPS, porta 3333):"
...
info "Banco (email):  ${BANK_PLATFORM_EMAIL}"
...
# tsx (global) — set EARLY even though food uses node (keeps pattern consistent)
...
# Garantir vite-env.d.ts existe (for client build)
if [ ! -f "$APP_CWD/client/src/vite-env.d.ts" ]; then
    mkdir -p "$APP_CWD/client/src"
    echo '/// <reference types="vite/client" />' > "$APP_CWD/client/src/vite-env.d.ts"
...
info "Instalando dependencias do server (raiz)..."
cd "$APP_CWD"
npm install 2>&1 | tail -3
ok "Server deps instaladas"

info "Instalando dependencias do client..."
cd "$APP_CWD/client"
npm install 2>&1 | tail -3
...
# client/ tem seu proprio node_modules (install separado)
node "$APP_CWD/client/node_modules/vite/bin/vite.js" build 2>&1 | tail -5
...
if grep -rq "localhost" "$APP_CWD/client/dist/assets/"*.js 2>/dev/null; then
    warn "localhost encontrado no build — verificar configuracao"
fi

ok "Frontend em client/dist/"
...
if [ -f "$APP_CWD/.env" ]; then
    ...
    BANK_WEBHOOK_SECRET="foodflow-webhook-secret-$(openssl rand -hex 8)"

    echo "# ECP Digital Food — PRODUCAO ($(date '+%Y-%m-%d %H:%M'))
NODE_ENV=production
PORT=${APP_PORT}
HOST=127.0.0.1
...
ECP_PAY_API_KEY=ecp-food-dev-key" > "$APP_CWD/.env"
...
# ============================================================================
# ETAPA 8: Banco de dados (seed)
# ============================================================================
step "8/12" "Banco de dados"

mkdir -p "$APP_CWD/data"

SKIP_SEED=""
if [ -f "$APP_CWD/data/foodflow.db" ]; then
    if ask_yes_no "Banco ja existe. Recriar? (APAGA DADOS)" "n"; then
        rm -f "$APP_CWD/data/foodflow.db"
...
if [ -z "$SKIP_SEED" ]; then
    ...
    node server/seed.mjs 2>&1 | tail -5
    ok "Banco populado"
fi
...
# Food uses node (plain .mjs), NOT tsx
echo "module.exports={apps:[{name:'${APP_NAME}',script:'node',args:'server/index.mjs',cwd:'${APP_CWD}',instances:1,exec_mode:'fork',env_production:{NODE_ENV:'production',PORT:${APP_PORT},HOST:'127.0.0.1'},max_memory_restart:'512M',max_restarts:10,min_uptime:'10s',restart_delay:4000,kill_timeout:5000,listen_timeout:10000,log_date_format:'YYYY-MM-DD HH:mm:ss Z',merge_logs:true,cron_restart:'0 4 * * *'}]};" > "$APP_CWD/ecosystem.config.cjs"
...
server {
    ...
    # API proxy (passthrough, no rewrite)
    location /api/ {
        proxy_pass http://ecp_food_backend;
    ...
    location /assets/ {
        alias /opt/ecp-digital-food/03-product-delivery/client/dist/assets/;
        expires 1y;
    ...
    # SPA fallback
    location / {
        root /opt/ecp-digital-food/03-product-delivery/client/dist;
        try_files $uri $uri/ /index.html;
    }

    add_header X-Frame-Options DENY always;
    add_header X-Content-Type-Options nosniff always;
    access_log /var/log/nginx/ecp-digital-food-access.log;
    error_log /var/log/nginx/ecp-digital-food-error.log;
}
NGX

ln -sf /etc/nginx/sites-available/${APP_NAME} /etc/nginx/sites-enabled/

# Remove old foodflow config if exists
rm -f /etc/nginx/sites-enabled/foodflow /etc/nginx/sites-available/foodflow 2>/dev/null || true
...
H=$(curl -s -o /dev/null -w "%{http_code}" "http://127.0.0.1:${APP_PORT}/api/health" 2>/dev/null || echo "000")
[ "$H" = "200" ] && ok "API: porta ${APP_PORT}" || { fail "API: sem resposta (HTTP ${H})"; ERRORS=$((ERRORS+1)); }

[ -f "$APP_CWD/data/foodflow.db" ] && ok "Banco: $(du -h "$APP_CWD/data/foodflow.db" | cut -f1)" || { fail "Banco: nao encontrado"; ERRORS=$((ERRORS+1)); }
[ -f "$APP_CWD/client/dist/index.html" ] && ok "Frontend: build OK" || { fail "Frontend: sem build"; ERRORS=$((ERRORS+1)); }
systemctl is-active --quiet nginx && ok "Nginx: rodando" || { fail "Nginx: parado"; ERRORS=$((ERRORS+1)); }

EXT=$(curl -s -o /dev/null -w "%{http_code}" "http://${DOMAIN}/api/health" 2>/dev/null || echo "000")
...
if [ -z "${SKIP_SSL:-}" ] && [ -f "/etc/letsencrypt/live/${DOMAIN}/fullchain.pem" ]; then
    EXTS=$(curl -s -o /dev/null -w "%{http_code}" "https://${DOMAIN}/api/health" 2>/dev/null || echo "000")
